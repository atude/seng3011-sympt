...
//  ?keyterms=something[,somethingelse,hi]&startdate=yyyy:mm:ddTHH:MM:SS&enddate=yyyy:mm:ddTHH:MM:SS&location=somewhere
...
const promedURLResults = async (urlSearchQueries: string) => {
  ...
  if (
    ...
    // lol fix this later
    console.log("incorrect api request");
  ...
  const formattedKeyTerms: string =
    keyTerms?.split(",").join(" AND ") + " AND " + location;
  ...
  if (startDateGroups != null) {
    startYear = startDateGroups[1];
    startMonth = startDateGroups[2];
    startDay = startDateGroups[3];
  }
  ...
  if (endDateGroups != null) {
    endYear = endDateGroups[1];
    endMonth = endDateGroups[2];
    endDay = endDateGroups[3];
  }
  //format date ready for input into promed
